...
| LLM account | **Claude is the supported client** — custom connectors are configured once on claude.ai and sync to the iOS/Android/Desktop apps, right for a phone-first author. Verified July 2026: the **Free plan supports exactly one custom connector** (all we need) and up to 5 Projects, so Marinda pilots at no cost; upgrade to Pro ($20/mo) only if Free's ~30–100-messages-per-5-hours ceiling interrupts real interviews (draft checkpointing makes an interrupted interview resumable, not lost). ChatGPT is a documented degraded fallback only (verified against OpenAI docs: custom write-capable MCP connectors require beta Developer Mode, are web-only — no mobile apps — and write actions require confirmation dialogs, rememberable per-conversation at best). |
